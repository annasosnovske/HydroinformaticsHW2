...
def processSNOTEL(site, stateab, WYOI, sitedf=None):
    ...
    if sitedf is None:
        ...
        if os.path.exists(local_csv):
            sitedf = pd.read_csv(local_csv)
        else:
            # Fall back to the public source when local files have not been created yet.
            remote_csv = f"https://raw.githubusercontent.com/egagli/snotel_ccss_stations/main/data/{site}.csv"
            sitedf = pd.read_csv(remote_csv)
            os.makedirs("files/SNOTEL", exist_ok=True)
            sitedf.to_csv(local_csv, index=False)

    # Normalize the public station CSV format to the legacy columns expected below.
    if 'Water_Year' not in sitedf.columns:
        if {'datetime', 'WTEQ'}.issubset(set(sitedf.columns)):
            temp = sitedf.copy()
            temp['Date'] = pd.to_datetime(temp['datetime'])
            temp['Water_Year'] = temp['Date'].dt.year + (temp['Date'].dt.month >= 10).astype(int)
            # Public CSV WTEQ is in inches; convert to meters for compatibility.
            temp['Snow Water Equivalent (m) Start of Day Values'] = pd.to_numeric(temp['WTEQ'], errors='coerce').fillna(0) / 39.3701
            sitedf = temp[['Date', 'Water_Year', 'Snow Water Equivalent (m) Start of Day Values']].copy()
        else:
            raise ValueError(
